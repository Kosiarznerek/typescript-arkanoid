const KONTROLER: Kontroler = new Kontroler(5);

//Eventy na window
window.addEventListener('load', () => {
    Promise.all(new Array()
        .concat(PrzeladujGrafiki())
        .concat(PrzeladujPoziomy(KONTROLER.maxPoziomow))
    ).then(poWczytaniu)
});
...
let EL_GRY: ElementyGry;

/**
 * Obsługa klawiatury
 * @param {KeyboardEvent} e
 */
function klawiszDol(e: KeyboardEvent): void {
 ...
 * @param {KeyboardEvent} e
 */
function klawiszGora(e: KeyboardEvent): void {
    switch (e.key.toUpperCase()) {
        case 'A':
...
function animacja(): void {
    ...
    EL_GRY.paletka.aktualizujPozycje(
        new Array()
    ...
    EL_GRY.paletka.aktualizujPociski(
        ...
        new Array()
            ...
            .concat(EL_GRY.barierki)
            .concat(EL_GRY.bramy)
    ).forEach((value: Przeciwnik | Cegielka | Barierka | Brama) => {
        //Zestrzelone obiekty
    ...
    EL_GRY.kulki.forEach(value => value
        //Dla każdej kulki aktualizuje pozycje
            .aktualizujPozycje(
                new Array()
            ...
            //Dla każdego elementu w jaki uderzyła kulka
            .forEach((value2: Kulka | Paletka | Cegielka | Przeciwnik | Barierka | Brama) => {
                ...
                if (value2 instanceof Cegielka) {
                    ...
                    if (value2.jestRozwalona && Math.randomInt(1, 100) <= 20) {
                        EL_GRY.fiolki.push(new Fiolka(
                            value2.pozycja.klonuj(),
                            TYPY_FIOLEK[Math.randomKey(TYPY_FIOLEK)]
                        ));
                    }
                    if (value2.jestRozwalona) KONTROLER.punktyGracza += 100;
                }
                ...
                if (value2 instanceof Paletka && value.pozycja.y + value.wysokosc <= value2.pozycja.y) {
                    ...
                    const kierunek: Wektor = new Wektor;
                    kierunek.y = -1;
                    const rozX: number = value.srodek.x - value2.srodek.x;
    ...
    EL_GRY.paletka.dotknela(EL_GRY.fiolki).forEach(value => {
        ...
        switch (value.typ) {
            ...
            case TYPY_FIOLEK.C:
                //Dodaje dwie kulki
                const pozycja1: Wektor = EL_GRY.kulki[Math.randomInt(0, EL_GRY.kulki.length - 1)].pozycja.klonuj();
                for (let i = 0; i < 2; i++) EL_GRY.kulki.push(new Kulka(pozycja1, new Wektor(0, 1)));
                break;
            case TYPY_FIOLEK.D:
    ...
    EL_GRY.paletka.dotknela(EL_GRY.przeciwnicy).forEach(value => {
        ...
        EL_GRY.paletka.rozwal(() => {
            ...
            if (KONTROLER.zyciaGracza === 0) {
                ...
                KONTROLER.resetuj();
                EL_GRY = GenerujPoziom(KONTROLER.obecnyPoziom);
            } else EL_GRY.paletka.wskrzes()
        });
    });

    //Jeżeli na ekranie 0 kulek -> gracz umiera
    if (EL_GRY.kulki.length === 0) EL_GRY.paletka.rozwal(() => {
        KONTROLER.zyciaGracza--;
        if (KONTROLER.zyciaGracza === 0) {
            //Brak żyć -> gra od nowa
            KONTROLER.resetuj();
            ...
            EL_GRY.kulki.push(new Kulka(
                new Wektor(EL_GRY.paletka.srodek.x, EL_GRY.paletka.pozycja.y),
                new Wektor(0, 1)
            ))
        }
    });
    ...
    EL_GRY.przeciwnicy = EL_GRY.przeciwnicy.filter(value => {
        ...
        if ((value.jestZabity || !value.jestNaEkranie(EKRAN.GRA)) && EL_GRY.bramy.filter(value2 => value2.jestZamknieta).length > 0) {
            ...
            const otwarte: Brama[] = EL_GRY.bramy.filter(value2 => value2.jestZamknieta);
            const wylosowana: Brama = otwarte[Math.randomInt(0, otwarte.length - 1)];
            wylosowana.otworz(() => {
                const przeciwnik: Przeciwnik = new Przeciwnik(new Wektor, TYPY_PRZECIWNIKOW[Math.randomKey(TYPY_PRZECIWNIKOW)]);
                przeciwnik.pozycja = new Wektor(
                    wylosowana.srodek.x - przeciwnik.szerokosc / 2,
                    wylosowana.pozycja.y + wylosowana.wysokosc
                );
