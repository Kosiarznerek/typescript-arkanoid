...
//Zmienne canvasu
let CTX;
const EKRAN = new Prostokat(new Punkt(0, 0), 540, 652);
//Elementy HTML
const HTML = {
...
function wcisniecieKlawisza(klawisz) {
    ...
    if (klawisz.toUpperCase() === 'P')
        PRZECIWNICY.push(new Kolo(5, MYSZ.klonuj()));
    //Usuwanie klockow
...
function GenerujJSON() {
    //Filtruje bloki
    const bloki = BIERZACE_BLOKI.filter(value => value.typ !== DOSTEPNE_BLOKI.PALETKA);
    let paletki = BIERZACE_BLOKI.filter(value => value.typ === DOSTEPNE_BLOKI.PALETKA);
    let paletka;
    if (paletki.length !== 1) {
...
function WczytajJSON() {
    ...
    try {
        ...
        //Dodaje paletke
        BIERZACE_BLOKI.push(new Blok(new Punkt(json.paletka.pozycja.x, json.paletka.pozycja.y), DOSTEPNE_BLOKI.PALETKA));
        //Zczytuje przeciwnikow
        PRZECIWNICY = json.przeciwnicy.map(value => new Kolo(5, new Punkt(value.x, value.y)));
    }
    catch (e) {
...
function animacja() {
    ...
    if (OBECNIE_WYBRANY) {
        ...
        Kolizja.AktualizujProstokatUnikajac(OBECNIE_WYBRANY, w, new Array()
